...
BUILD_INFER="${BUILD_INFER:-0}"
TOKENIZER="${TOKENIZER:-models/hf/unsloth-Llama-3.2-1B-Instruct/tokenizer.json}"

mkdir -p "$OUT_DIR"
STAMP="$(date +%Y%m%d_%H%M%S)"
RAW_CSV="$OUT_DIR/llama32_gguf_compare_${STAMP}.csv"
SUMMARY_MD="$OUT_DIR/llama32_gguf_compare_${STAMP}_summary.md"

MODELS=(
  "llama32-1b-bf16-from-gguf.cellm|models/llama32-1b-bf16-from-gguf.cellm"
  "llama32-1b-bf16-q4k-from-gguf.cellm|models/llama32-1b-bf16-q4k-from-gguf.cellm"
  "llama32-1b-bf16-q6k-from-gguf.cellm|models/llama32-1b-bf16-q6k-from-gguf.cellm"
)
...
for spec in "${MODELS[@]}"; do
  IFS='|' read -r model_name model_path <<< "$spec"
  for backend in $BACKENDS; do
    for pass in $(seq 1 "$PASSES"); do
      echo "running: model=$model_name backend=$backend pass=$pass/$PASSES"
      run_one "$model_name" "$model_path" "$backend" "$pass"
    done
  done
done
...
{
  echo "Llama-3.2 1B GGUF-derived benchmark (${PASSES} passes, host run on $(date '+%B %-d, %Y'); prompt=\`\"$PROMPT_TEXT\"\`, \`--gen $GEN_TOKENS\`, report = median / P95):"
  echo "| Model | Backend | Startup (s) | Prefill (s) | Decode (s) |"
  echo "|---|---|---:|---:|---:|"
  for spec in "${MODELS[@]}"; do
    IFS='|' read -r model_name _ <<< "$spec"
    for backend in $BACKENDS; do
